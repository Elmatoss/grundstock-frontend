			...
			<div className="mt-6 grid gap-4 sm:grid-cols-2">
				<Card title={m.helfen_shift_title()}>
					...
					<a
						...
						className="mt-4 inline-block btn-primary rounded-xs px-5 py-2.5 text-sm font-semibold no-underline"
					>
						{m.helfen_shift_cta()}
					</a>
				</Card>
				<Card title={m.helfen_mittel_title()}>
					...
					<MittelgschaftlerDialog>
						<button
							type="button"
							className="mt-4 inline-block cursor-pointer rounded-xs border border-glow/50 px-5 py-2.5 text-sm font-semibold text-glow-soft transition-colors hover:border-glow hover:text-glow"
						>
							{m.helfen_mittel_cta()}
						</button>
					</MittelgschaftlerDialog>
